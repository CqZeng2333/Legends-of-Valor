...
public class Hero extends Avatar implements HeroSkillsInterface {
	...
	public int loseHowMuchHP(double num) {
		if (num < this.defense())
			return 0; // cannot hurt the hero
		else
			return (int) (num - this.defense());
	}
	...
	/*
	 * The hero buy an object with specific level and money. Return -1 if no enough
	 * money; return -2 if no enough level; return -3 if both not enough; return 0
	 * if successfully buy the object.
	 */
	...
	/*
	 * Add a buyable object to the hero return 0 if success return -1 if not correct
	 * type
	 */
	...
	/*
	 * Sell the object from the hero's outfit Return 0 if success return -1 if not
	 * correct type return -2 if no such object
	 */
	...
	/*
	 * According to "type"(weapon/armor) to do exchange Return 0 if success; return
	 * -1 if no such equipment; return -2 if "type" not weapon or armor
	 */
	...
	public String toString() {
		return "Name: " + name + " Type: " + type + " Level: " + level + " HP: " + HP + " Mana: " + mana + " EXP: "
				+ exp + " Money: " + money + " Strength: " + skills.get("strength").intValue() + " Agility: "
				+ skills.get("agility").intValue() + " Dexterity: " + skills.get("dexterity").intValue();
	}

	public String toStringInFight() {
		return "Name: " + name + " Type: " + type + " Level: " + level + " HP: " + HP + " Mana: " + mana + " EXP: "
				+ exp + " Money: " + money + " Weapon: "
				+ (outfit.getWeaponEquipped().size() > 0 ? outfit.getWeaponEquipped().get(0).getName() : "None")
				+ " Armor: "
				+ (outfit.getArmorEquipped().size() > 0 ? outfit.getArmorEquipped().get(0).getName() : "None");
	}
